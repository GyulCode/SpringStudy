     ...
     <form method="post" action="../member/join_ok.do" @submit="submitForm()">
      <table class="table">
        ...
        <tr>
         ...
         <td width=90% class="inline">
          <input type=password name=pwd ref="pwd" v-model="pwd" size=20 class="input-sm"
          	@keyup="pwdValidate"
          >
          &nbsp;재입력:
          <input type=password name=pwd1 ref="pwd1" v-model="pwd1" size=20 class="input-sm"
          	@keyup="pwdValidate2"
          >
          <p>{{pwdOk}}</p>
         </td>
 	...
 	new Vue({
 		...
 		methods:{
 			...
 			idCheckValidate:function(id){
 				...
 				let num=id_res.search(/[0-9]/g) /* id의 숫자가 있느냐 js함수임 js정규식*/
 				let eng=id_res.search(/[a-z]/ig)
 				if(id_res.length<6 || id.length>12)
				{
 					this.idOk='아이디는 6~12자리 이내로 입력하세요'	
 				}
 				else if(id_res.search(/\s/)!=-1){
 					//공백 찾기
 					this.idOk='아이디 공백 없이 입력해 주세요'
 					return;
 				}
 				else if(num<0 || eng<0){
 			...
 			pwdValidate:function(){
 				...
 				if(pwd.length<8 || pwd.length>20){
 					this.pwdOk='비밀번호는 8~20자 사이로 입력해 주세요'
 					return;
 				}
 				else if(pwd.search(/\s/)!=-1){
 					this.pwdOk='공백없이 입력해주세요!!'
 					return;
 				}
 				else if(num<0 || eng<0){
 					this.pwdOk='비밀번호는 영문, 숫자를 혼합해 주세요'
 					return;
 				}
 				else{
 					this.pwdOk='';
