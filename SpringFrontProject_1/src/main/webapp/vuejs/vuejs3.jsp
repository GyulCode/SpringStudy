...
<body>
	<div class=container>
		...
		<div class=row >
			<table class=table>
				<tr>
					<td>
						<input type=text size=30 class="input-sm" v-model="msg" ref="msg"> <!-- id가 아닌 ref 사용 -->
						<input type=button class="btn btn-primary btn-sm" value="입력" 
						v-on:click="find()"><!-- find 사용자 정의 함수 -->
					</td>
				</tr>
			</table>
			<table class=table>
				...
				<tbody>
					<tr v-for="vo in movie">
						...
						<td class="text-center">
							<img alt="" :src="'https://www.kobis.or.kr/'+vo.thumbUrl" style="width:30px; hieght:30px;">
						</td>
	...
	new Vue({
		...
		beforeCreate:function(){
			...
			// componenetWillCreate (React)
		},
		created:function(){
			console.log("Create:Vue객체 생성완료...")
			// componenetDidCreate (React)
		},
		beforeMount:function(){
			console.log("beforeMount:가상 메모리(가상돔) 올리기전...")
		},
		mounted:function(){
			console.log("Mounted:가상메모리에 HTML을 올리기완료 ...")
			// $(function()), window.onload
			//가상돔 / 실제 돔을 비교해서 연결 diffing
			//가장많이 사용되는 상태(서버에서 데이터를 읽어오는 경우)
			
		},
		beforeUpdate:function(){
			console.log("beforeUpdate:상태가 변경(데이터값 변경)되기 직전상태 ...")
			//멤버 변수 설정 : react=>state, Vue=data => data를 다른 메소드에서 ...
			
		},
		updated:function(){
			console.log("updated: 데이터 변경이나 수정이 돤료된 상태 -> mounted를 재호출...")
			
		},
		beforeDestroy:function(){
			console.log("beforeDestroy: 메모리 해제전 상태...")
